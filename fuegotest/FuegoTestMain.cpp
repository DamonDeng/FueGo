...
int main(int argc, char** argv)
{
    SgDebug() << "Starting the testing new. With args:" << argv << " parameter number: " << argc << " \n";

    SgUctTree m_tree;

    m_tree.CreateAllocators(1);
    
    m_tree.SetMaxNodes(10000);

    

    ostringstream debugResult;

    m_tree.DumpDebugInfo(debugResult);

    SgDebug() << debugResult.str() << ". \n";
